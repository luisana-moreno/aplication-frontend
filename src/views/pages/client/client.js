        ...
        <CRow
            ...
            <CCol md={6}>
                <CFormInput
                    ...
                    onChange={(e) => setAddClient({ ...addClient, firts_type: e.target.value })}
                />
                <small
                    className="text-muted">
                    Please add your client type.
                </small>
            </CCol>
...
const Client = () => {
    ...
    const handleNextEdit = () => {
        if (currentEditSectionClient < sections.length - 1) {
            setCurrentEditSectionClient(currentSectionClient + 1);
        }
    };
        ...
        <CCard>
            ...
            <CCardBody>
                <CTable hover responsive>
                    <CTableHead>
                        <CTableRow>
                            <CTableHeaderCell className='text-green'> <CIcon icon={cilTag} />Client type</CTableHeaderCell>
                            <CTableHeaderCell className='text-green'> <CIcon icon={cilTags} />Company Name</CTableHeaderCell>
                            <CTableHeaderCell className='text-green'> <CIcon icon={cilTag} />firts name</CTableHeaderCell>
                            ...
                            <CTableHeaderCell className='text-green'> <CIcon icon={cilCalendar} />Rif</CTableHeaderCell>
                    ...
                    <CTableBody>
                        {client.map((client) => (
                            <CTableRow key={client.Document_Number}>
                                ...
                                <CTableDataCell>{client?.Middle_Name || ''} </CTableDataCell>
                                <CTableDataCell>{client?.Firts_Las_Name || ''} </CTableDataCell>
                                <CTableDataCell>{client?.Second_Las_Name || ''} </CTableDataCell>
                                <CTableDataCell>{client?.Document_Number || ''} </CTableDataCell>
                                <CTableDataCell>{client?.Date_Birth || ''} </CTableDataCell>
                                <CTableDataCell>{client?.Phone || ''} </CTableDataCell>
                                <CTableDataCell>{client?.Address || ''} </CTableDataCell>
                                <CTableDataCell>{client?.Contact_Person || ''} </CTableDataCell>
                                ...
                                <CTableDataCell>
                                    ...
                                    <CButton
                                        ...
                                        color='danger'
                                        variant='outline'
                                        onClick={() => { setCurrentClient(client); setDeleteVisibleClient(true); }}>
                                        Delete
                                    </CButton>
            ...
            <CModal
                visible={deleteVisibleClient}
                onClose={() => setDeleteVisibleClient(false)}
            >
                <CModalHeader>
                    <CModalTitle
                ...
                <CModalBody>
                    <h6>
                        Please write 'confirm' to delete this client
                    </h6>
                    <CFormInput
                        placeholder="confirm"
                        className='modal-border'
                        value={deleteConfirmationClient}
                        onChange={(e) => setDeleteConfirmationClient(e.target.value)} /> {/* detectar los cambios guardados */}
                </CModalBody>

                <CModalFooter>
                    <CButton
                        className='button-no-hover-green'
                        onClick={() => setDeleteVisibleClient(false)}>
                        <h6
